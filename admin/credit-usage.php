...
$pages = max(1, (int)ceil($total / $perPage)); $page = min($page, $pages); $offset = ($page - 1) * $perPage;
  $summaryStmt = $db->prepare("SELECT COUNT(*) occurrences, COALESCE(SUM(CAST(JSON_UNQUOTE(JSON_EXTRACT(ul.meta, '$.tokens_used')) AS UNSIGNED)),0) tokens, COALESCE(SUM(ul.credits_used),0) credits FROM usage_logs ul LEFT JOIN users u ON u.id=ul.user_id{$whereSql}"); $summaryStmt->execute($params); $summary = $summaryStmt->fetch() ?: ['occurrences'=>0,'tokens'=>0,'credits'=>0];
...
<div class="card">
  ...
  <?php foreach ($usage as $row): $meta=usage_meta($row); $conversion=$meta['conversion'] ?? []; $tokensUsed=(int)($meta['tokens_used'] ?? 0); ?>
    <tr><td><?= format_datetime($row['logged_at']) ?></td><td><strong><?= h($row['user_name'] ?? '—') ?></strong><br><small class="text-muted"><?= h($row['user_email'] ?? '—') ?></small></td><td><span class="badge badge-secondary"><?= h($row['event_type']) ?></span></td><td><?= number_format($tokensUsed) ?></td><td><strong><?= number_format((float)$row['credits_used'], 6, ',', ' ') ?></strong></td><td><small>100k tokens = <?= number_format((float)($conversion['credits_per_unit'] ?? 1), 6, ',', ' ') ?> crédit(s)<br><?= number_format((float)($conversion['xof_per_unit'] ?? 120), 2, ',', ' ') ?> XOF</small></td><td><details><summary>Voir</summary><pre style="max-width:320px;white-space:pre-wrap;font-size:.75rem;margin:8px 0 0"><?= h(json_encode($meta['payload'] ?? $meta, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)) ?></pre></details></td></tr>
  <?php endforeach; ?>
