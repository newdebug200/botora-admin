...
<div class="row g-4">
  <div class="col-12 col-xl-4"><div class="card border-0 shadow-sm"><div class="card-body"><h2 class="h5">Ajuster un solde</h2><p class="small text-muted">Utilisez une valeur positive pour créditer et négative pour débiter.</p><form method="POST" class="vstack gap-3"><select name="user_id" class="form-select" required><option value="">Choisir un utilisateur</option><?php foreach($users as $u): ?><option value="<?= (int)$u['id'] ?>"><?= h($u['name']) ?> — <?= h($u['email']) ?> (<?= number_format($u['credits_balance'],3,',',' ') ?>)</option><?php endforeach; ?></select><input name="amount" type="number" step="0.0000000001" class="form-control" placeholder="Ex. 10.000 ou -2.500" required><input name="reason" class="form-control" value="Ajustement administrateur"><button class="btn btn-success">Enregistrer l’ajustement</button></form></div></div></div>
