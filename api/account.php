...
try {
  $db = db();
  $stmt = $db->prepare('SELECT id,name,email,company,phone,license_key,status,credits_balance,plan_id,trial_started_at,trial_ends_at,trial_used,subscription_started_at,subscription_ends_at,created_at,updated_at FROM users WHERE email=? LIMIT 1');
  $stmt->execute([$email]);
  $user = $stmt->fetch();
