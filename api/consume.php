...
try {
  ...
  $db->prepare('INSERT INTO usage_logs (user_id,event_type,credits_used,meta) VALUES (?,?,?,?)')->execute([$user['id'],$event_type,$amount,json_encode($meta)]);
  $db->commit();
} catch (Exception $e) {
  $db->rollBack();
...
api_json(['ok'=>true,'credits_balance'=>$new_balance,'consumed'=>$amount]);
